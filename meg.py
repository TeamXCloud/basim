...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 59:
                response = "❕Coldown in effect please wait 59 seconds❕"
                bot.reply_to(message, response)
                return
...
def welcome_start(message):
    ...
    response = f"❄️𝗪𝗘𝗟𝗖𝗢𝗠𝗘 𝗧𝗢 𝗕𝗚𝗠𝗜 𝗗𝗗𝗢𝗦 𝗕𝗢𝗧❄️"
    bot.reply_to(message, response)
